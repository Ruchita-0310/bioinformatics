...
with open('blastn_taxonomy.csv', mode='w', newline='') as keys_file:
    ...
    # Add column names
    value = ['query_id','subject_id','%identity','alignment_length','mismatches','gap_open','q_start','q_end','s_start','s_end','evalue','bit_score','prokaryote/virus','database_identifier','taxonomy','TRUE/FALSE','ref_seq_no']
    keys_writer.writerow(value)

    # Iterate over the lookup keys and perform a lookup in the data dictionary
    for row in rows:
        # need 22609 from p~22609~1527~lcl|NZ_CP054306.1_rrna_28~~1489~27 which is in column 2
        key = row[1].split('~')[1]
